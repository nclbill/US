   ...
   if ($action == 'ajouter') {
    /*     $db->query("INSERT INTO produits (categorie, marque, modele, version, couleur, vin, entrepot) VALUES (?, ?, ?, ?, ?, ?, ?)", [
            $categorie, $marque, $modele, $version, $couleur, $vin , $entrepot
				...
				if (empty($vin)) {
				    $err = array("Le VIN est obligatoire.");
				    display_errors($err);
						exit;
				} else {
    ...
    // Modifier un produit
			...
			if (empty($vin)) {
					...
					exit;
															...
															$query_vin_modif = $db->query("SELECT * FROM produits WHERE vin = ?", [$vin]);
															$count_vin_modif = $query_vin_modif->count();
																	if($count_vin_modif > 0){
																		$err = array("VIN existe deja parmi vos Produits");
																			display_errors($err);

																} else {
																	$query_id_product = $db->query("SELECT id FROM produits WHERE vin = ?", [$vin]);
