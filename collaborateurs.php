...
$ville = $results_collaborateur[0]->ville;
	}
////////////////////// fin search //////////////////////////////
	?>
...
<?php 			$query_all_collaborateur = $db->query("SELECT * FROM users  WHERE  habilitation <> ? and habilitation <> ? and habilitation <> ? and habilitation <> ?",["Admin","Utilisateur","","Client"]);
				 		$count_all_collaborateur = $query_all_collaborateur->count();
				 		$results_all_collaborateur = $query_all_collaborateur->results();
				 	//	$id_collaborateur = $results_collaborateur[0]->id;
					//	$id_user = $results_collaborateur[0]->id_user;

		//			dump($results_all_collaborateur);
?>
				<div class="row">
					<div class="col-sm-12">
						<br>
						<h2>vous avez <?=$count_all_collaborateur?> collaborateur<?php if($count_all_collaborateur != 1){ echo "s";}	?>
						</h2>
							...
							<tbody>
								<?php foreach ($results_all_collaborateur as $r) { ?>
									<tr>
										...
										<td>
											<a href="collaborateurs.php?searchTerm=<?=$r->cin_pass?>&id_collaborateur=<?=$r->id?>">Select</a>  <!--" Suprimer L'affectation "-->
										</td>
									</tr>
								<?php } ?>
							</tbody>
